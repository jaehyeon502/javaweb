...
export default function BoardWriteView() {
  ...
  //          Event Handler          //
  const onBoardContentChangeHandler = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const value = event.target.value;
    console.log(value);
    setBoardContent(value);
  }

  const onBoardContentKeyPressHandler = (event: KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>) => {
...
//          Use Effect          //
  ...
  return (
    <Box sx={{ p: '0 198px', backgroundColor: 'rgba(0, 0, 0, 0.05)' }}>
      <Box sx={{ p: '100px 24px', backgroundColor: '#ffffff' }}>
        <Input fullWidth disableUnderline placeholder="제목을 입력하세요." sx={{ fontSize: '30px', fontWeight: '500' }} onChange={(event) => setBoardTitle(event.target.value)} />
        <Divider sx={{ m: '40px 0' }} />
