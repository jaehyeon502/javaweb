...
INSERT INTO Reservation VALUES(2,'이영희', '2022-10-11', 908);
...
# MySQL 데이터타입
# 문자열 타입 CHAR 
# 고정길이 문자열 / 길이로 지정할 수 있는 값의 범위는 0 ~ 255
...
# Reservation 테이블에서 name이 '고길동'이면서 note 컬럼에 값이 지정되어 있지 않은 모든 컬럼을 선택
SELECT * FROM Reservation WHERE name = '고길동' AND note IS NULL;

# Reservation 테이블에서 note 컬럼에 값이 지정되어 있지 않고 \
# reservationDate가 2023-01-01부터 2023-02-28까지인 레코드의 모든 컬럼을 선택
SELECT * 
FROM Reservation 
WHERE note IS NULL 
...
# SELECT 컬럼명 FROM 테이블1명 INNER JOIN 테이블2명 ON JOIN 조건
...
# FROM 절에 두 개 이상의 테이블을 사용할 때 Alias를 사용해서 별칭을 보여할 수 있음
SELECT * FROM Reservation AS RV, Room AS RM WHERE RV.roomNumber = RM.roomNumber;

# 두 테이블 이상을 FROM 절에서 사용할 때는
# 선택할 컬럼명 앞에 어떤 테이블의 컬럼인지를 직접 지정 해 주는 것이 좋음
# 동일한 컬럼명이 각 테이블에 존재하면 쿼리는 어떤 테이블의 컬럼인지 구분 하지 못함
